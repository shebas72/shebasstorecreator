...
class Common_model extends CI_Model {
    ...
    //     $sshpassword = 'New@Store5000New@New';
    //     $mysqlsshpassword = 'New@Store5000New@New';
    //     $outPut = shell_exec("echo '".$sshpassword."' | sudo -S /usr/bin/bash /var/www/storecreator.io/virtualhost-subdomain create ".$domainname);
    //     $outPut = shell_exec("echo '".$sshpassword."' | sudo -S cp -R /var/www/storecreator.io/script/* /var/www/".$domainfolder."/");
    //     $outPut = shell_exec("echo '".$sshpassword."' | sudo -S chmod -R 0777 /var/www/".$domainfolder."/*");
    ...
    //     $sshpassword = 'New@Store5000New@New';
    //     $mysqlsshpassword = 'New@Store5000New@New';
    //     $outPut = shell_exec("echo '".$sshpassword."' | sudo -S /usr/bin/bash /var/www/storecreator.io/domaindns create ".$domainname);

    //     $outPut = shell_exec("echo '".$sshpassword."' | sudo -S /usr/bin/bash /var/www/storecreator.io/virtualhost create ".$domainname);
    //     $outPut2 = shell_exec("echo '".$sshpassword."' | sudo -S /usr/bin/certbot -d ".$domainname." --no-redirect");
    //     $outPut = shell_exec("echo '".$sshpassword."' | sudo -S cp -R /var/www/storecreator.io/script/* /var/www/".$domainfolder."/");
    //     $outPut = shell_exec("echo '".$sshpassword."' | sudo -S chmod -R 0777 /var/www/".$domainfolder."/*");

    // }
        
    function create_sub_domain_new($domainname){
        $sshpassword = 'New@Store5000New@New';
        $mysqlsshpassword = 'New@Store5000New@New';
        $outPut = shell_exec("echo '".$sshpassword."' | sudo -S /usr/bin/bash /var/www/storecreator.io/virtualhost-subdomain create ".$domainname);
    }
    function create_domain_new($domainname){
        $sshpassword = 'New@Store5000New@New';
        $mysqlsshpassword = 'New@Store5000New@New';
        $outPut = shell_exec("echo '".$sshpassword."' | sudo -S /usr/bin/bash /var/www/storecreator.io/domaindns create ".$domainname);
    ...
    function upgrade_domain_new($themefolder, $oldsqlsentax, $olddomainlink, $domainfolder, $domainlink, $sqlsentax){

        $sshpassword = 'New@Store5000New@New';
        $mysqlsshpassword = 'New@Store5000New@New';

        $outPut = shell_exec("echo '".$sshpassword."' | sudo -S /usr/bin/bash /var/www/storecreator.io/domaindns create ".$domainlink);

        $outPut = shell_exec("echo '".$sshpassword."' | sudo -S /usr/bin/bash /var/www/storecreator.io/virtualhost create ".$domainlink);
        $outPut2 = shell_exec("echo '".$sshpassword."' | sudo -S /usr/bin/certbot -d ".$domainlink." --no-redirect");


        $sqldb = str_replace('storecreatorio','',$domainfolder);
        $themesqldb = str_replace('storecreatorio','',$themefolder);


        $outPut2 = shell_exec("echo '".$sshpassword."' | sudo -S mysql -u root -pNew@Store5000New@New -e 'DROP database ".$sqlsentax.";'");
        $outPut2 = shell_exec("echo '".$sshpassword."' | sudo -S mysql -u root -pNew@Store5000New@New -e 'CREATE database ".$sqlsentax.";'");

        $outPut2 = shell_exec("echo '".$sshpassword."' | sudo -S mysqldump -u root -pNew@Store5000New@New ".$oldsqlsentax." > /var/www/storecreator.io/sql/".$oldsqlsentax.".sql");
        $outPut2 = shell_exec("echo '".$sshpassword."' | sudo -S replace '".$olddomainlink."' '".$domainlink."' -- /var/www/storecreator.io/sql/".$oldsqlsentax.".sql");
        $outPut2 = shell_exec("echo '".$sshpassword."' | sudo -S mysql -u root -pNew@Store5000New@New ".$sqlsentax." < /var/www/storecreator.io/sql/".$oldsqlsentax.".sql");


        $outPut = shell_exec("echo '".$sshpassword."' | sudo -S cp -R /var/www/".$themefolder."/* /var/www/".$domainfolder."/");

        $outPut = shell_exec("echo '".$sshpassword."' | sudo -S /usr/bin/bash /var/www/storecreator.io/createconfig ".$domainfolder." ".$sqlsentax);
        $outPut = shell_exec("echo '".$sshpassword."' | sudo -S /usr/bin/bash /var/www/storecreator.io/createhtaccess ".$domainfolder);
        $outPut = shell_exec("echo '".$sshpassword."' | sudo -S chmod 0777 /var/www/".$domainfolder."/*");
        $outPut = shell_exec("echo '".$sshpassword."' | sudo -S wget -O/dev/null -q ".$domainlink."/active-plugin.php");
        $outPut = shell_exec("echo '".$sshpassword."' | sudo -S chown -R apache:apache /var/www/".$domainfolder."/*");
    ...
    function delete_domain($domainname, $domainfolder, $sqlsentax){
        $sshpassword = 'New@Store5000New@New';
        $mysqlsshpassword = 'New@Store5000New@New';
        $outPut = shell_exec("echo '".$sshpassword."' | sudo -S rm -rf /var/www/".$domainfolder."/*");
        // $outPut = shell_exec("echo '".$sshpassword."' | sudo -S /usr/bin/bash /var/www/storecreator.io/virtualhost delete ".$domainname." --y");
    ...
    function insert_into_table_wp_mysql($sqlsentax,$domainname,$email){
        ...
        $mysql_username = 'root';
        $mysql_password = $mysqlsshpassword;
    ...
    function create_wp_config_file($folderlocation, $sqldb){
        $sshpassword = 'New@Store5000New@New';
        $mysqlsshpassword = 'New@Store5000New@New';
        $outPut = shell_exec("echo '".$sshpassword."' | sudo -S /usr/bin/bash /var/www/storecreator.io/createconfig ".$folderlocation." ".$sqldb);
    }
    function htaccess_wp_file($folderlocation){
        $sshpassword = 'New@Store5000New@New';
        $mysqlsshpassword = 'New@Store5000New@New';
        $outPut = shell_exec("echo '".$sshpassword."' | sudo -S /usr/bin/bash /var/www/storecreator.io/createhtaccess ".$folderlocation);
        $outPut = shell_exec("echo '".$sshpassword."' | sudo -S chmod 0777 /var/www/".$folderlocation."/.htaccess");
    }
    ...
    function tempfunction(){
        $this->db->select('*');
        $this->db->from('code_domain b');
        $this->db->join('code_theme t','b.domaintitle = t.themetitle','LEFT');
        $this->db->where('b.domainuser','1');
        $this->db->where('t.tempcol','2');
        $this->db->limit(20);
    ...
    function copy_files($themefolder, $domainfolder, $domainlink, $sqlsentax){
        ...
        // Name of the file
        $filename = '/var/www/'.$themefolder.'/wordpress.sql';
        $mysql_host = 'localhost';
        $mysql_username = 'root';
        $mysql_password = $mysqlsshpassword;
        $mysql_database = $sqlsentax;
        ...
        foreach ($lines as $line) {
            $strline = str_replace(
                array($themesqldb.".storecreator.io"),
                array($domainlink),
                $line
            );
            if (substr($strline, 0, 2) == '--' || $line == '')
    ...
    function copy_plugin_files($appzip, $domainfolder, $domainlink, $activeplugin){
        ...
        $outPut = shell_exec("echo '".$sshpassword."' | sudo -S cp -R /var/www/storecreator.io/plugins/".$appzip." /var/www/".$domainfolder."/wp-content/plugins/");
        $outPut2 = shell_exec("echo '".$sshpassword."' | sudo -S unzip /var/www/".$domainfolder."/wp-content/plugins/".$appzip." -d /var/www/".$domainfolder."/wp-content/plugins/".$appfolder);

        echo file_get_contents("https://".$domainlink."/active-plugin.php?pluginfilename=".$activeplugin);
    }
