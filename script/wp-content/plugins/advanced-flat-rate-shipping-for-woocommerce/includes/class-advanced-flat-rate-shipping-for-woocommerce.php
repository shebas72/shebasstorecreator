...
class Advanced_Flat_Rate_Shipping_For_WooCommerce_Pro {
    ...
    public function __construct() {
        ...
        add_filter("{$prefix}plugin_action_links_" . AFRSM_PRO_PLUGIN_BASENAME, array($this, 'plugin_action_links'), 10, 4);
    }
    ...
    public function afrsm_init_shipping_method() {
        ...
        require_once AFRSM_PLUGIN_DIR . '/admin/partials/afrsm-forceall-shipping-method.php';
    ...
    private function define_admin_hooks() {
        ...
        $this->loader->add_action('admin_head', $plugin_admin, 'afrsm_pro_active_menu');
        $this->loader->add_action('admin_head', $plugin_admin, 'afrsm_pro_remove_admin_submenus');

        $this->loader->add_action('afrsm_condition_match_rules', $plugin_admin, 'afrsm_pro_condition_match_rules', 10, 2);
        
        $this->loader->add_action('wp_ajax_afrsm_pro_subscribe_newsletter', $plugin_admin, 'afrsm_pro_subscribe_newsletter');

        $this->loader->add_action('wp_ajax_sm_sort_order', $plugin_admin, 'afrsm_pro_sm_sort_order');
        $this->loader->add_action('wp_ajax_nopriv_sm_sort_order', $plugin_admin, 'afrsm_pro_sm_sort_order');

        $this->loader->add_action('wp_ajax_save_master_settings', $plugin_admin, 'afrsm_pro_save_master_settings');
        ...
        $this->loader->add_action('wp_ajax_nopriv_afrsm_pro_product_fees_conditions_values_ajax', $plugin_admin, 'afrsm_pro_product_fees_conditions_values_ajax');
	     ...
	     $this->loader->add_action('wp_ajax_afrsm_pro_product_fees_conditions_varible_values_product_ajax', $plugin_admin, 'afrsm_pro_product_fees_conditions_varible_values_product_ajax');
	    $this->loader->add_action('wp_ajax_nopriv_afrsm_pro_product_fees_conditions_varible_values_product_ajax', $plugin_admin, 'afrsm_pro_product_fees_conditions_varible_values_product_ajax');
        ...
        if (!empty($_GET['page']) && (($_GET['page'] == 'afrsm-pro-list') || ($_GET['page'] == 'afrsm-pro-add-shipping') || ($_GET['page'] == 'afrsm-pro-edit-shipping') || ($_GET['page'] == 'wc-shipping-zones') || ($_GET['page'] == 'afrsm-pro-get-started') || ($_GET['page'] == 'afrsm-pro-information') )) {
    ...
    private function define_public_hooks() {
        ...
        $this->loader->add_action('woocommerce_calculate_totals', $plugin_public, 'afrsm_pro_update_chosen_shipping_method_total');
    ...
    public function plugin_action_links($actions, $plugin_file, $plugin_data, $context) {
        $custom_actions = array(
            'configure' => sprintf('<a href="%s">%s</a>', admin_url('admin.php?page=afrsm-pro-list'), __('Settings', $this->plugin_name)),
            'docs' => sprintf('<a href="%s" target="_blank">%s</a>', esc_url('store.multidots.com/docs/plugin/advanced-flat-rate-shipping-method-for-woocommerce'), __('Docs', $this->plugin_name)),
            'support' => sprintf('<a href="%s" target="_blank">%s</a>', esc_url('store.multidots.com/dotstore-support-panel'), __('Support', $this->plugin_name))
        );
