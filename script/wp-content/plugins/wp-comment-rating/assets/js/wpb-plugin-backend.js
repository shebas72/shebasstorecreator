jQuery(document).ready(function () {
	...
	/************************************************************
	 * Use WordPress' new media editor to select an image
	 ************************************************************/

	var custom_uploader;

	jQuery('.wpb-mediaselect').click(function (e) {
		e.preventDefault();
		...
		custom_uploader.on('select', function () {
			...
			/* This is needed because of the retina logo
			 * You will upload a retina image but only use the width and height of the "normal" logo image
			 * Okay... I know. This is tricky :-)
			 */
			if (button.parent().find('input').attr('name') == 'schemacorporate_logo_url') {
	...
	/************************************************************
	 * CHOOSEN jQuery Plugin (Multiselect)
	 ************************************************************/
	...
	jQuery('.chzn-select-ajax').each(function () {
	...
	/************************************************************
	 * HIDE/SHOW WHATS IMPORTANT AT THE MOMENT (ON/OFF Button)
	 ************************************************************/

	function do_the_switch(e) {
		var related_items = e.data('related_items');
		var not_related_items = e.data('not_related_items');

		var select_switch;

		for (var related_item in related_items) {
			if (e.find('input').is(':checked')) {
				e.parent().parent().parent().find('tr:eq(' + related_items[related_item] + ')').show();
			}
			else {
				e.parent().parent().parent().find('tr:eq(' + related_items[related_item] + ')').hide();
			}
		}

		for (var not_related_item in not_related_items) {
			if (e.find('input').is(':checked')) {
				e.parent().parent().parent().find('tr:eq(' + not_related_items[not_related_item] + ')').hide();
			}
			else {
				e.parent().parent().parent().find('tr:eq(' + not_related_items[not_related_item] + ')').show();
			}
		}
	}

	jQuery('.onoffswitch input').click(function () {
		var related_table = jQuery(this).parent().parent().parent().parent();
		var related_items = jQuery(this).parent().data('related_items');
		var not_related_items = jQuery(this).parent().data('not_related_items');

		var select_switch, onoffswitch, has_switch = false;

		if (jQuery(this).is(':checked')) {
			for (var related_item in related_items) {
				related_table.find('tr:eq(' + related_items[related_item] + ')').show();
			}

			for (var related_item in related_items) {

				/* Search for another switch */
				onoffswitch = related_table.find('tr:eq(' + related_items[related_item] + ')').find('.onoffswitch');
				if (0 != onoffswitch.length) {
					/* If a switch was found, do the switch */
					do_the_switch(onoffswitch);
				}

				if (!related_table.find('tr:eq(' + related_items[related_item] + ')').is(':hidden')) {
					select_switch = related_table.find('tr:eq(' + related_items[related_item] + ')').find('.wpbplugin-related');
					if (0 != select_switch.length) {
						/* If a select was found, do the switch */
						do_the_select_switch(select_switch);
					}
				}
			}
		} else {
			for (var related_item in related_items) {
				related_table.find('tr:eq(' + related_items[related_item] + ')').hide();
			}
		}

		for (var not_related_item in not_related_items) {
			if (jQuery(this).is(':checked')) {
				related_table.find('tr:eq(' + not_related_items[not_related_item] + ')').hide();
			}
			else {
				related_table.find('tr:eq(' + not_related_items[not_related_item] + ')').show();
				select_switch = related_table.find('tr:eq(' + not_related_items[not_related_item] + ')').find('.wpbplugin-related');
				if (0 != select_switch.length) has_switch = true;
			}
		}
	...
	jQuery('.onoffswitch').each(function () {
		var related_items = jQuery(this).data('related_items');

		var onoffswitch, select_switch;

		for (var related_item in related_items) {
			element = jQuery(this).parent().parent().parent().find('tr:eq(' + related_items[related_item] + ')');
			if (jQuery(this).find('input').is(':checked') && !element.is(':hidden')) {
				element.show();

				onoffswitch = element.find('.onoffswitch');
				if (0 != onoffswitch.length) {
					/* If a switch was found, do the switch */
					do_the_switch(onoffswitch);
				}

				select_switch = element.find('.wpbplugin-related');
				if (0 != select_switch.length) {
					/* If a select was found, do the switch */
					do_the_select_switch(select_switch);
				}
	...
	/************************************************************
	 * HIDE/SHOW WHATS IMPORTANT AT THE MOMENT (SELECT-ELEMENTS)
	 ************************************************************/



	function do_the_select_switch(e) {
		var related_items = e.data('related_items');
		var selected = e.val();
		related_items = related_items[selected];

		var not_related_items = e.data('not_related_items');
		not_related_items = not_related_items[selected];
	...
	jQuery('.wpbplugin-related').change(function () {
		var related_table = jQuery(this).parent().parent().parent();
		var related_items = jQuery(this).data('related_items');
		var selected = jQuery(this).val();
		related_items = related_items[selected];

		var not_related_items = jQuery(this).data('not_related_items');
		not_related_items = not_related_items[selected];
	...
	jQuery('.wpbplugin-related').each(function () {
		var related_items = jQuery(this).data('related_items');
		var selected = jQuery(this).val();
		related_items = related_items[selected];
		...
		for (var not_related_item in not_related_items) {
			element = jQuery(this).parent().parent().parent().find('tr:eq(' + not_related_items[not_related_item] + ')');
			element.hide();
		}
	});
	...
	if (jQuery.isFunction(jQuery.fn.wpColorPicker)) {
		jQuery('input.wpb-color-picker').wpColorPicker();
	}
	...
	return $.fn.ajaxChosen = function (settings, callback) {
		...
		return this.each(function () {
			return $(this).next('.chzn-container').find(".search-field > input, .chzn-search > input").bind('keyup', function () {
				...
				/*
				 * @extended wp-buddy.com
				 * Check for data in any of the data-html attributes
				 */
				var undefined;
				if (options.data.action === undefined) {
					options.data.action = select.data('ajax_action');
				}
				if (options.data._wpbplugin_nonce_name === undefined) {
					options.data._wpbplugin_nonce_name = select.data('ajax_nonce');
				}
				/**
				 * @end/extended
				 */
