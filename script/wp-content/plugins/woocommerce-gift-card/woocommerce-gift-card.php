...
class SB_WCGifts
{
	...
	public function addActions()
	{
		if( is_admin() ):
		    ...
		    add_action('save_post',array($this, 'save_order_note')); 
	...
	{
		add_meta_box('gift-metbox', __('Receiver Details'), array($this, 'metabox_gift'), 'shop_order');
	}
	...
	public function wgc_metabox()
	{
	    ...
	    <h2><?php _e('Gift Card Settings - WwPpLlOoCcKkEeRr..CcOoMm','wgc'); ?></h2>
...
// style --	.woocommerce ul.products li.first, .woocommerce-page ul.products li.first {clear:none !important}
	...
	public function actionPaymentComplete($order_id)
	{
		...
		$is_sent = get_post_meta($order_id, '_gift_email_sent');
		if( $is_gift == 'yes' && $is_sent[0] != 'yes')
		{
		    $admin_mail = get_option( 'admin_email' );
		    wp_mail($to,$subject, $message, array("From: $blogname <$admin_mail>"));
		    update_post_meta($order_id, '_gift_email_sent', 'yes'); //Mark email as sent to prevent duplicate emails
	    }
	}
	...
	public function metabox_gift($post)
	{
		$order = new WC_Order($post->ID);
		$rname =  get_post_meta($order->id, '_gift_receiver_name', 1);
		$remail = get_post_meta($order->id, '_gift_receiver_email', 1);

		foreach($order->get_items() as $item_id => $item)
		{
			//check if order item is a product gift
			$is_gift = get_post_meta($item['product_id'], '_gift', 1);
			if( $is_gift != 'yes' ) return false;
		}
		...
		<div class="woocommerce_order_items_wrapper">
			...
			<thead>
			<tr>
				<th class="item"><?php _e('Recipient\'s Email');?></th>
				<th class="quantity"><?php _e('Recipient\'s Name'); ?></th>
				<th class="line_cost"><?php _e('Action'); ?></th>
			</tr>
			</thead>
			<tbody>
				<tr>
				<td><input type="text" name="wgc_name" value="<?php print $rname; ?>" /></td>
				<td><input type="text" name="wgc_email" value="<?php print $remail; ?>" /></td>
				<td><input type="submit" name="wgc_submit" value="Resend Email" /></td>
				</tr>
			</tbody>
			</table>
		</div>
	...
	public function save_order_note($post_id)
	{
		if( !isset($_POST['post_type']) || $_POST['post_type'] != 'shop_order')
			return false;
		if(isset($_POST['wgc_submit']))
		{
			update_post_meta($post_id, '_gift_email_sent', 'no');
			update_post_meta($post_id, '_gift_receiver_name', trim(@$_POST['wgc_name']));
			update_post_meta($post_id, '_gift_receiver_email', trim(@$_POST['wgc_email']));
			self::actionPaymentComplete($post_id);
			//die('dead');
		}
	}
