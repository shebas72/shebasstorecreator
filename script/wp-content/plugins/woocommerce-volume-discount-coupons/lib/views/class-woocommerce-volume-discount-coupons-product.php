...
class WooCommerce_Volume_Discount_Coupons_Product {
	...
	public static function render_info( $product ) {
		...
		if ( $coupons && ( count( $coupons ) > 0 ) ) {
			$product_term_ids = array_map( 'intval', wp_get_post_terms( $product->get_id(), 'product_cat', array( 'fields' => 'ids' ) ) );
			foreach ( $coupons as $coupon ) {
				...
				if ( in_array( intval( $product->get_id() ), $coupon_product_ids ) ||
					...
					if ( $show_description || $show_info ) {
						...
						if ( $coupon->get_id() ) {
							if ( $show_description ) {
								if ( $post = get_post( $coupon->get_id() ) ) {
									if ( !empty( $post->post_excerpt ) ) {
										$output .= sprintf( '<div class="coupon display description %s">', wp_strip_all_tags( $coupon->get_code() ) );
							...
							if ( $show_info ) {
								$output .= sprintf( '<div class="coupon display volume-discount %s">', wp_strip_all_tags( $coupon->get_code() ) );
