...
class WooCommerce_Volume_Discount_Coupons_Shortcodes {
	...
	public static function coupon_code( $atts, $content = null ) {
		...
		foreach ( $codes as $code ) {
			$coupon = new WC_Coupon( $code );
			if ( $coupon->get_id() ) {
				$output .= sprintf( '<span class="coupon code %s">', wp_strip_all_tags( $coupon->get_code() ) );
			...
			if ( $coupon->get_id() ) {
				if ( $post = get_post( $coupon->get_id() ) ) {
					if ( !empty( $post->post_excerpt ) ) {
						$output .= sprintf( '<span class="coupon description %s">', wp_strip_all_tags( $coupon->get_code() ) );
	...
	public static function coupon_volume_discount( $atts, $content = null ) {
		...
		foreach ( $codes as $code ) {
			$coupon = new WC_Coupon( $code );
			if ( $coupon->get_id() ) {
				$output .= sprintf( '<span class="coupon volume-discount %s">', wp_strip_all_tags( $coupon->get_code() ) );
	...
	public static function get_volume_discount_info( $coupon, $atts = array() ) {
		...
		$vd_product_ids = get_post_meta( $coupon->get_id(), '_vd_product_ids', false );
		$vd_term_ids = get_post_meta( $coupon->get_id(), '_vd_term_ids', false );
		$min         = get_post_meta( $coupon->get_id(), '_vd_min', true );
		$max         = get_post_meta( $coupon->get_id(), '_vd_max', true );
		$auto        = get_post_meta( $coupon->get_id(), '_vd_auto', true );
		...
		switch ( $coupon->get_discount_type() ) {
			...
			case 'percent_product' :
				if ( count( $coupon->get_product_ids() ) > 0 ) {
					...
					$discount = sprintf( __( '%s%s discount on selected products', WOO_VOLDISC_PLUGIN_DOMAIN ), $amount, $amount_suffix );
				} else if ( $coupon->get_discount_type() == 'percent' ) {
		...
		if ( $auto || !$show_using_code ) {
			...
			if ( $min > 0 && $max > 0 ) {
				if ( $min != $max ) {
					$result = sprintf( __( 'Buy %1$d to %2$d %3$s and get a <span class="coupon discount">%4$s</span> using the coupon code <span class="coupon code">%5$s</span>', WOO_VOLDISC_PLUGIN_DOMAIN ), $min, $max, $vd_list, $discount, $coupon->get_code() );
				} else {
					$result = sprintf( __( 'Buy %1$d %2$s and get a <span class="coupon discount">%3$s</span> using the coupon code <span class="coupon code">%4$s</span>', WOO_VOLDISC_PLUGIN_DOMAIN ), $min, $vd_list, $discount, $coupon->get_code() );
				}
			} else if ( $min > 0 ) {
				$result = sprintf( __( 'Buy %1$d or more %2$s and get a <span class="coupon discount">%3$s</span> using the coupon code <span class="coupon code">%4$s</span>', WOO_VOLDISC_PLUGIN_DOMAIN ), $min, $vd_list, $discount, $coupon->get_code() );
			} else if ( $max > 0 ) {
				$result = sprintf( __( 'Buy up to %1$d %2$s and get a <span class="coupon discount">%3$s</span> using the coupon code <span class="coupon code">%4$s</span>', WOO_VOLDISC_PLUGIN_DOMAIN ), $max, $vd_list, $discount, $coupon->get_code() );
			}
		}
