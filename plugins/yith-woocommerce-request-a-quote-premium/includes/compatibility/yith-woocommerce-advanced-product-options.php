...
if ( ! class_exists( 'YWRAQ_Avanced_Product_Options' ) ) {
	...
	class YWRAQ_Avanced_Product_Options {
		...
		public function __construct() {
			...
			//add_filter( 'ywraq_add_to_cart_from_request', array( $this, 'add_to_cart_from_request' ), 10, 4 );
			add_action( 'ywraq_from_cart_to_order_item', array( $this, 'add_order_item_meta' ), 10, 3 );
			//add_filter( 'ywraq_cart_to_order_args' , 'cart_to_order_args' , 10 , 2 );
		...
		public function ajax_add_item( $postdata, $product_id ) {
            ...
            $yith_wapo_is_sold_individually = isset( $postdata['yith_wapo_is_single'] ) ? $postdata['yith_wapo_is_single'] : false;

			$t = $yith_wapo_frontend->add_cart_item_data( null, $product_id, $postdata, $yith_wapo_is_sold_individually );
		...
		public function request_quote_view( $item_data, $raq, $_product, $show_price = true ) {
			if ( isset( $raq['yith_wapo_options'] ) ) {
				foreach ( $raq['yith_wapo_options'] as $_r ) {
					...
					//wc 2.7
					if( is_callable( $_product, 'adjust_price') ){
